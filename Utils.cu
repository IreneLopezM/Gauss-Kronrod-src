...
void Newton(double *x, double *w, int n, double x1, double x2, double cdc){
	int m,j,i;
	double z1,xi,xm,xl,pp,p3,p2,p1;
	m=(n+1)/2;
	xm=(x1+x2)/2;
	xl=(x2-x1)/2;
	for(i=0;i<m;i++){
		...
		do{
			...
			for(j=0;j<n;j++){
				p3=p2;
				p2=p1;
				p1=((2.0*j+1)*xi*p2-j*p3)/(j+1);
			}

			pp=n*(xi*p1-p2)/(xi*xi-1);
			z1=xi;
			xi=z1-(p1/pp);
		}while(fabs(xi-z1)>cdc);

		x[i]=xm-(xl*xi);
		x[n-(i+1)]=xm+(xl*xi);
		w[n-(i+1)]=w[i]=(2.0*xl)/((1.0-xi*xi)*pp*pp);
	}
}
...
void kronrod_sh (int n, double eps, double z[], double w[])
{
    ...
    w1  = (double *)calloc(n+1, sizeof(double));
